...
**Version alignment.** OpenNVR's components now share one release number:
this repo's adapter images, `opennvr-app-sdk` and the OpenNVR platform all
ship as 0.5.0 from this release on. Nothing about the adapter images
changed to earn the jump from 0.1.4 — the number moved so an operator can
read one version across the stack instead of three. The standalone
`opennvr-adapter-sdk` keeps its own line (1.2.0) because it is a library
consumed outside this repo, versioned by its own API contract.
