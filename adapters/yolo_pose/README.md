...
`onnxslim` is in the list because ultralytics exports with `simplify=True` by
default and calls it to slim the graph. It is **optional**, and deliberately
listed anyway: the slim step is wrapped in a `try`/`except` upstream, so without
onnxslim the export logs `simplifier failure` and still produces a working
`.onnx` — just an unslimmed one, which is not the graph the throughput numbers
below were measured on. Ultralytics tries to install it for you first, which in
an ephemeral environment shells out to a `pip` that may not be on PATH (a
harmless `returned non-zero exit status 127` warning), so pinning it is what
makes the export reproducible rather than environment-dependent.

[`adapters/yolov8/`](../yolov8/README.md) takes the other road and exports with
`simplify=False`, which skips the whole question. Either is fine; this adapter
slims because it is CPU-first and its latency budget is the tight one.
